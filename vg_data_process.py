...
if dataset == 'vg' and data_type == 'pred':
    ...
    roidb_path = '/NMP/data/vrd_roidb_adjusted.npz'  #
    res_path = '/home/cuishuai0607/NMP-master/data/pretrained/vg_vgg_pretrained.ckpt'  
    N_each_batch = 30
    is_rela = False
    ...
    roidb_path = '/NMP/data/vrd_roidb_adjusted.npz'
    res_path = '/NMP/data/pretrained/vg_vgg_pretrained.ckpt'
    N_each_batch = 128
    is_rela = True
elif dataset == 'vrd' and data_type == 'pred':
    # ---------- vrd pred dataset---------------#
    if use_ori_vgg:
        save_path = os.path.join(feat_save_path, 'ori_vg_vgg_feats')
    else:
        save_path = os.path.join(feat_save_path, 'vg_vgg_feats')
    roidb_path = '/NMP/data/vg_roidb_adjusted.npz'
    res_path = '/NMP/data/pretrained/vrd_vgg_pretrained.ckpt'
    N_each_batch = 30
    is_rela = False

    # ---------- vrd rela dataset  ----------#
    if use_ori_vgg:
        save_path = os.path.join(feat_save_path, 'ori_vg_rela_vgg_feats')
    else:
        save_path = os.path.join(feat_save_path, 'vg_rela_vgg_feats')
    roidb_path = '/NMP/data/vg_roidb_adjusted.npz'
    res_path = '/NMP/data/pretrained/vg_vgg_pretrained.ckpt'
